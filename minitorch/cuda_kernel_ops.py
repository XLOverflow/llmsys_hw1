...
class CudaKernelOps(TensorOps):
    ...
    def layernorm_fw(inp: Tensor, gamma: Tensor, beta: Tensor):
      ...
      ln_res = tensor_from_numpy(np.zeros(inp.shape, dtype=datatype), backend=inp.backend)
      var = tensor_from_numpy(np.zeros((batch_size,), dtype=datatype), backend=inp.backend)
      mean = tensor_from_numpy(np.zeros((batch_size,), dtype=datatype), backend=inp.backend)
    ...
    def layernorm_bw(out_grad: Tensor, inp: Tensor, gamma: Tensor, beta: Tensor, var: Tensor, mean: Tensor):
      ...
      gamma_grad = tensor_from_numpy(np.zeros(gamma.shape, dtype=datatype), backend=gamma.backend)
      beta_grad = tensor_from_numpy(np.zeros(beta.shape, dtype=datatype), backend=beta.backend)
      inp_grad = tensor_from_numpy(np.zeros(inp.shape, dtype=datatype), backend=inp.backend)
